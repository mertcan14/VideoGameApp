...
struct GetDetailVideoGameRequest: BaseRequestProtocol {
    typealias Response = DetailVideoGameNetworkModel
    ...
    func decode(_ data: Data) throws -> DetailVideoGameNetworkModel {
        let decoder = JSONDecoder()
        let response = try decoder.decode(DetailVideoGameNetworkModel.self, from: data)
        return response
    }
}
